 ...
 * @subpackage	Components
 */
class potterViewUser extends JViewLegacy
{
	 	...
	 	} else
		{
			...
			$result = $db->loadObjectList();
         	...
         	}else
         	{
				...
				$query  = "SELECT * FROM `".$db_pre."users` WHERE block=0 ORDER BY name";
				$db->setQuery( $query );
				$joouser = $db->loadObjectList();

				$this->assignRef('darst', $result);
				$this->assignRef('rolls', $rolls);
				$this->assignRef('joouser', $joouser);
				parent::display($tpl);
         	}
		}
	}
